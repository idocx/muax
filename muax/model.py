  ...
  """
    ...
    """
        ...
        if with_pi and with_value:
            return action, plan_output.action_weights, root_value
        elif not with_pi and with_value:
            return action, root_value
        elif with_pi and not with_value:
            return action, plan_output.action_weights
        else:
            return action
    ...
    def optimizer_state(self, opt_state):
    ...
    def set_weights(self, weights):
        ...
        self._params, self._opt_state = weights
    ...
    def _init_policy(self, policy):
        if policy == 'muzero':
            ...
            return jax.jit(policy_func,
                           static_argnames=(
                           'temperature', 'recurrent_fn', 'num_simulations', 'loop_fn', 'qtransform', 'max_depth',
                           'dirichlet_fraction', 'dirichlet_alpha', 'pb_c_init', 'pb_c_base'),
                           backend='cpu')
        elif policy == 'gumbel':
            policy_func = mctx.gumbel_muzero_policy
            return jax.jit(policy_func, static_argnames=(
            'recurrent_fn', 'num_simulations', 'max_num_considered_actions', 'gumbel_scale', 'loop_fn', 'qtransform',
            'max_depth'),
                           backend='cpu')
        else:
